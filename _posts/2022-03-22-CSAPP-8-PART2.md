...
Ctrl+C를 입력하면 `sigint_handler` 함수가 실행된다.<br /><br />
...
Linux는 blocking signal에 대한 implicit, explicit mechanism을 제공한다.<br />
...
signal이 또 오면, 그 signal은 pending되지만 S가 일을 마치고 return해도 received되지 않는다. <br />
