...
`...`는 _ellipsis_ 라고 불리는 token으로, 가변이 아닌 parameter 뒤에 마지막으로 나온다.<br />
사용 예시는 아래와 같다.
>``` C
>int i=100; int j=50;
...
즉, 추가로 붙은 모든 argument들이 __VA_ARGS__ 자리에 자동으로 들어가게 된다.<br />
### The __func__ Identifier
__func__ identifier는 preprocessor이 처리하는 부분은 아니지만 기능이 유사하고 디버깅에 쓰기 좋아서 이 챕터에서 소개되었다.<br />
