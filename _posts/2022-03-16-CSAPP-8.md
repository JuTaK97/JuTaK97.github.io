...
수 있고 시스템의 어떤 메모리 주소에도 접근할 수 있다. 반대로 _user mode_ 의 process는 이러한 _privileged instruction_ 에는 접근하지 못한다. mode bit를 바꾸거나 CPU를 
...
운영체제의 kernel이 multitasking 하는 방법은 여러 개의 process들을 concurrent하게 진행시키는 것이었다. 여기서 _context switch_ 라는 exception control flow를 사용한다.
